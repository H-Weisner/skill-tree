...
describe('Skill Tree Store', () => {
  ...
  describe('Graph Logic & Cycles', () => {
    it('prevents circular dependencies', () => {
      ...
      expect(store.edges).toHaveLength(1) // Should not have added the bad edge
    })
  })
  ...
  describe('Unlock Logic', () => {
    ...
    it('allows unlocking a node if parent is unlocked', () => {
      ...
      store.unlockNode(nodeA) // Unlock parent

      const result = store.unlockNode(nodeB) // Try unlock child
      expect(result.success).toBe(true)
    })
  })
